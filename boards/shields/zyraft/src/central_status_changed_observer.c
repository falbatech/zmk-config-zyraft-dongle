 ...
 *
 * Adapted from carrefinho/prospector-zmk-module
 * Fires zmk_split_central_status_changed with slot + connected
 * when a split peripheral connects or disconnects.
 */
...
static int release_psptr_peripheral_slot_for_conn(struct bt_conn *conn) {
    int idx = psptr_peripheral_slot_index_for_conn(conn);
    if (idx < 0) {
        return idx;
    }
...
static int zmk_split_bt_central_status_init(void) {
    bt_conn_cb_register(&conn_callbacks);
    return 0;
}

SYS_INIT(zmk_split_bt_central_status_init, APPLICATION, CONFIG_ZMK_BLE_INIT_PRIORITY);
